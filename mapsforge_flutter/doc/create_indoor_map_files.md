#  How to build indoor .map files from .osm files

**TODO**: How to build them from .osm.pbf files from Geofabrik
...
## Step 3: Prepare offline osm data

Usually osm data are downloaded from osm servers which add information at the upload. These additional information are necessary for the conversion process. If you want to convert a downloaded osm file which you have not touched (or only deleted nodes or ways) then you can skip this section and go on with step 4.

Otherwise if you added or modified anything yourself you have to follow the next sub-steps. It is recommended to copy your osm file before this step to prevent irreversible damage to the file as you will have to manually edit it.

### Step 3.1: Add the missing attributes

OSM files are just more or less standardized xml files with a header for general information as well well as all nodes, ways, relations - each containing additional information (attributs). By uploading the osm file to an osm server, the file receives a set of modified and additional attributs for all nodes, ways, and relations which are required by the mapfile writer to convert properly. Here is a list of attributes that should be in all nodes, ways, and relations:
- an id > 0   [`id="20465986"`]
- a visible statement (must be true !)   [`visible="true"`]
...
- maybe a username   [`user="JohnDoe"`]
- maybe a user-id   [`uid="1234"`] 

So if your data is missing some attributes the easiest way to add/modify them is by using the find-and-replace function of a text editor like [Xcode](https://apps.apple.com/de/app/xcode/id497799835?mt=12) (Mac) or [Notepad++](https://notepad-plus-plus.org/downloads/) (Windows). Therefor, change the name of your .osm file to a .xml at first to open it correctly. Look for some nodes or ways and check which attributes are missing. Via find-and-replace you can add them like in the following example.
...
We can remove the `action` key if the value is not `"delete"` as it is of no use anymore. Therefor, find all `action="modify"` and replace it just with the missing attributes: `timestamp="2021-05-23T07:16:19Z" uid="12345678" user="JohnDoe" visible="true" version="1" changeset="123456789"`. Our example will look like this now:
```
...
Do not forget to undo the renaming back from .xml to .osm.

### Step 3.1: Add the missing boundary box

Just like the version attributes your file is missing information, usually provided by a server when downloading. If the first 3 rows of your osm or respectivly xml file looks like this:
```
...
<?xml version="1.0" encoding="UTF-8"?>
<osm version="0.6" generator="JOSM">
    <bounds minlat="50.8412220" minlon="12.9259499" maxlat="50.8419171" maxlon="12.9280428"/>
    <node id="104099" timestamp='2021-05-23T07:16:19Z' uid='12345678' user='JonDoe' visible='true' version='1' changeset='123456789' lat="50.84176020006" lon="12.92692686547" />
```
...
We can also tell the writer to use the hard drive for converting as we have limited RAM by default: `type=hd` 
...
If you have the parameter `type=hd` set to the mapfile-writer, remove it.
