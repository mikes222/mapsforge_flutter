...
import 'package:mapsforge_flutter/implementation/graphics/fluttertilebitmap.dart';
...
class DummyRenderer extends JobRenderer {
  ...
  Future<TileBitmap> executeJob(Job job) async {
    ...
    canvas.drawLine(
        ui.Offset.zero,
        ui.Offset(job.tile.tileSize.toDouble(), job.tile.tileSize.toDouble()),
        paint);
    canvas.drawLine(ui.Offset(job.tile.tileSize.toDouble(), 0),
        ui.Offset(0, job.tile.tileSize.toDouble()), paint);
    ...
    canvas.drawParagraph(
        builder.build()
          ..layout(
              ui.ParagraphConstraints(width: job.tile.tileSize.toDouble())),
        Offset(0, 0));
