...
/// in the main thread.
class IsolateDatastoreReader with IsolateMixin<IsolateMapInitParam> {
  final bool useIsolate;

  IsolateDatastoreReader({required this.useIsolate});
  ...
  Future<IsolateMapReplyParams> read(
    ...
    if (useIsolate) {
      ...
      IsolateMapReplyParams params = await sendToIsolate(
          IsolateMapRequestParam(tile, projection, renderContext, renderTheme));
      return params;
    } else {
  ...
  Future<IsolateMapReplyParams> readLabels(
      Datastore datastore,
      Tile tile,
      PixelProjection projection,
      RenderContext renderContext,
...
class IsolateMapRequestParam extends IsolateRequestParams {
  ...
  const IsolateMapRequestParam(
      this.tile, this.projection, this.renderContext, this.renderTheme);
}
...
Future<IsolateMapReplyParams> _readMapDataInIsolate(
  ...
  if (mapReadResult != null)
    _processMapReadResult(
        isolateParam.renderContext, isolateParam.renderTheme, mapReadResult);
  return IsolateMapReplyParams(
...
void _processMapReadResult(final RenderContext renderContext,
...
List<RenderInstruction> _retrieveRenderInstructionsForWay(
  ...
  if (wayProperties.isClosedWay) {
    List<RenderInstruction> renderInstructions =
        renderTheme.matchClosedWay(renderContext.upperLeft, wayProperties.way);
    return renderInstructions;
  } else {
    List<RenderInstruction> renderInstructions =
        renderTheme.matchLinearWay(renderContext.upperLeft, wayProperties.way);
    return renderInstructions;
  }
}
