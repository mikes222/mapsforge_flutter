...
class DeprecatedDatabaseRenderer extends StandardRenderer {
   ...
   */
  DeprecatedDatabaseRenderer(MapDataStore mapDataStore, GraphicFactory graphicFactory, this.labelStore, this.renderLabels, bool cacheLabels,
  ...
  Future<TileBitmap> executeJob(RendererJob rendererJob) async {
    ...
    try {
      renderContext = new RenderContext(rendererJob, new CanvasRasterer(graphicFactory, 0, 0), null);

      if (renderBitmap(renderContext)) {
        TileBitmap bitmap = null;

        if (this.mapDataStore != null) {
          MapReadResult mapReadResult = await this.mapDataStore.readMapDataSingle(rendererJob.tile);
  ...
  TileBitmap createBackgroundBitmap(RenderContext renderContext) {
    TileBitmap bitmap = this.graphicFactory.createTileBitmap(renderContext.job.tile.tileSize, renderContext.job.hasAlpha);
    //renderContext.canvasRasterer.setCanvasBitmap(bitmap);
    if (!renderContext.job.hasAlpha) {
