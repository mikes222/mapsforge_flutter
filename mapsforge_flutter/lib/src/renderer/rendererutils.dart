...
class RendererUtils {
  ...
  static List<Mappoint?> parallelPath(List<Mappoint?> p, double dy) {
    int n = p.length - 1;
    List<Mappoint?> u = new List<Mappoint?>.filled(n, null);
    List<Mappoint?> h = new List<Mappoint?>.filled(n + 1, null);
    ...
    for (int k = 0; k < n; ++k) {
      ...
      if (l == 0) {
        u[k] = new Mappoint(0, 0);
      } else {
        u[k] = new Mappoint(c / l, s / l);
      }
    }

    // For the start point calculate the normal
    h[0] = new Mappoint(p[0]!.x - dy * u[0]!.y, p[0]!.y + dy * u[0]!.x);

    // For 1 to N-1 calculate the intersection of the offset lines
    for (int k = 1; k < n; k++) {
      double l = dy / (1 + u[k]!.x * u[k - 1]!.x + u[k]!.y * u[k - 1]!.y);
      h[k] = new Mappoint(p[k]!.x - l * (u[k]!.y + u[k - 1]!.y), p[k]!.y + l * (u[k]!.x + u[k - 1]!.x));
    }

    // For the end point use the normal
    h[n] = new Mappoint(p[n]!.x - dy * u[n - 1]!.y, p[n]!.y + dy * u[n - 1]!.x);
