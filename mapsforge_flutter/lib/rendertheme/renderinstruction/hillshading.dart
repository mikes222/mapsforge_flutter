...
class Hillshading {
  ...
  Hillshading(this.minZoom, this.maxZoom, this.magnitude, this.layer,
      this.always, this.level, GraphicFactory graphicFactory);
  ...
  void render(
    ...
    if (hillsRenderConfig == null) {
      if (always) {
        ...
        renderContext.addToCurrentDrawingLayer(
            level, new ShapePaintContainer(hillShape, null, 0));
      }
      return;
    }
    ...
    double maptileTopLat =
        MercatorProjection.pixelYToLatitude(origin.y, tile.mapSize);
    double maptileLeftLng =
        MercatorProjection.pixelXToLongitude(origin.x, tile.mapSize);

    double maptileBottomLat = MercatorProjection.pixelYToLatitude(
        origin.y + tile.tileSize, tile.mapSize);
    double maptileRightLng = MercatorProjection.pixelXToLongitude(
        origin.x + tile.tileSize, tile.mapSize);
    ...
    for (int shadingLeftLng = maptileLeftLng.floor();
      ...
      for (int shadingBottomLat = maptileBottomLat.floor();
        ...
        if (shadingTopLat > maptileTopLat) {
          ...
          maptileSubrectTop = MercatorProjection.latitudeToPixelYWithMapSize(
        ...
        if (shadingBottomLat < maptileBottomLat) {
          ...
          maptileSubrectBottom = MercatorProjection.latitudeToPixelYWithMapSize(
                  shadingBottomLat + (shadingPixelOffset / shadingInnerHeight),
                  tile.mapSize) -
              origin.y;
        }
        if (shadingLeftLng < maptileLeftLng) {
          ...
          maptileSubrectLeft = MercatorProjection.longitudeToPixelXAtMapSize(
        ...
        if (shadingRightLng > maptileRightLng) {
          ...
          maptileSubrectRight = MercatorProjection.longitudeToPixelXAtMapSize(
