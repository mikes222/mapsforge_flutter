...
class RenderTheme {
  ...
  List<Hillshading> hillShadings =
      new List(); // NOPMD specific interface for trimToSize
   ...
   * @param way
   */
  Future matchClosedWay(RenderCallback renderCallback,
      final RenderContext renderContext, PolylineContainer way) async {
   ...
   * @param way
   */
  void matchLinearWay(RenderCallback renderCallback,
  ...
  void matchNode(RenderCallback renderCallback,
    ...
    if (matchingList != null) {
      ...
      for (int i = 0, n = matchingList.length; i < n; ++i) {
        matchingList
            .elementAt(i)
            .renderNode(renderCallback, renderContext, poi);
      }
      return;
    }
    ...
    for (int i = 0, n = this.rulesList.length; i < n; ++i) {
      this
          .rulesList
          .elementAt(i)
          .matchNode(renderCallback, renderContext, matchingList, poi);
    }
...
//    this.hillShadings.trimToSize();
  ...
  void matchWay(RenderCallback renderCallback,
      final RenderContext renderContext, Closed closed, PolylineContainer way) {
    MatchingCacheKey matchingCacheKey = new MatchingCacheKey(
        way.getTags(), way.getUpperLeft().zoomLevel, closed);
    ...
    matchingList = new List<RenderInstruction>();
    for (int i = 0, n = this.rulesList.length; i < n; ++i) {
      this.rulesList.elementAt(i).matchWay(renderCallback, way,
          way.getUpperLeft(), closed, matchingList, renderContext);
    }
