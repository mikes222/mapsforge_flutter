...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(title: 'Mapsforge simple online renderer example', home: MyHomePage());
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(backgroundColor: Theme.of(context).colorScheme.inversePrimary, title: const Text('Mapsforge OnlineRenderer simple example')),
      body: Center(
        child: Padding(
  ...
  Future<MapModel> createModel(BuildContext context) async {
    ...
    _mapModel = await MapModelHelper.createOnlineMapModel(renderer: ArcgisOnlineRenderer(), zoomlevelRange: const ZoomlevelRange(0, 21));
