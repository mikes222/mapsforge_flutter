...
class OsmWriter {
  ...
  int writeNode(ILatLong node, TagholderCollection tags) {
    if (tags.isEmpty) {
  ...
  List<int> _writeNodesForWay(Waypath waypath) {
    ...
    if (closed) {
      int firstId = writeNode(waypath.path.first, TagholderCollection.empty());
      nodes.add(firstId);
      for (var latLong in waypath.path.skip(1).take(waypath.path.length - 2)) {
        nodes.add(writeNode(latLong, TagholderCollection.empty()));
      }
      nodes.add(firstId);
    } else {
  ...
  void _writeTags(TagholderCollection tags, IOSink sink) {
    ++_currentIdent;
  ...
  int _writeWay(List<int> nodes, TagholderCollection tags, IOSink sink) {
  ...
  int _writeRelation(List<int> members, List<int> outerMembers, TagholderCollection tags) {
  ...
  void _writeWays(IOSink sink) {
    for (var way in _ways) {
      ...
      for (List<int> nodes in way.nodes) {
        wayIds.add(_writeWay(nodes, !relationNeeded ? way.tags : TagholderCollection.empty(), sink));
      }
