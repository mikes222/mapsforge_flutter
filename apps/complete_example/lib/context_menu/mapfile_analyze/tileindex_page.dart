...
class TileindexPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Tile Index")),
      ...
      child: FutureBuilder(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError || snapshot.error != null) {
            return Center(
              child: Text(snapshot.error.toString(), style: const TextStyle(fontSize: 12, color: Colors.red)),
            );
          }
    ...
    wayCounts.forEach((zoomlevel, waycount) {
      for (int i = 0; i < waycount; ++i) {
        ...
        try {
          ...
          res.add(
            Text(
              "Error ${error.toString()} while reading way $i from position 0x${(subFileParameter.startAddress + offset + pos).toRadixString(16)}",
