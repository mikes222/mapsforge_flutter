...
class PoiMarker<T> extends AbstractPoiMarker<T> with CaptionMixin {
  ...
  Future<void> changeZoomlevel(int zoomlevel, PixelProjection projection) async {
    ...
    renderInfo = RenderInfoNode(nodeProperties, renderinstruction.forZoomlevel(zoomlevel));
    renderInfo!.shapePainter = await ShapePaintSymbol.create(renderinstructionZoomed);
  ...
  Future<void> setBitmapColorFromNumber(int color) async {
    ...
    renderInfo!.shapePainter = await ShapePaintSymbol.create(renderInfo!.renderInstruction);
  }
  ...
  RenderinstructionSymbol? searchForSymbol(String symbolId) {
    return renderInfo?.renderInstruction;
  }
}
