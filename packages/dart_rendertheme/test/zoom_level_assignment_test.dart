...
void main() {
  ...
  group('Zoom Level Assignment Tests', () {
    late RenderthemeZoomlevel renderThemeZoomlevel;

    setUpAll(() async {
      Rendertheme rendertheme = await RenderThemeBuilder.createFromFile("test/defaultrender.xml");
      renderThemeZoomlevel = rendertheme.prepareZoomlevel(15);
    });

    test('should have exactly 260 levels (0-259) for defaultrender.xml', () {
      expect(renderThemeZoomlevel.maxLevels, equals(259));
    });
    ...
    test('should assign levels correctly within rules', () {
      ...
      _collectLevelsFromRules(renderThemeZoomlevel.rulesList, allLevels, levelsList);
    ...
    test('should assign same level to all render instructions within same rule', () {
      _verifyRuleLevelConsistency(renderThemeZoomlevel.rulesList);
    });

    test('should assign levels in generally ascending order', () {
      List<int> documentOrderLevels = <int>[];
      _collectLevelsInDocumentOrder(renderThemeZoomlevel.rulesList, documentOrderLevels);
    ...
    test('should demonstrate level assignment behavior', () {
      ...
      _mapLevelsToMultipleRules(renderThemeZoomlevel.rulesList, levelToRulesMap, '');

      // Print some statistics for understanding the actual behavior
      print('Total unique levels: ${levelToRulesMap.length}');
      print('MaxLevels from theme: ${renderThemeZoomlevel.maxLevels}');
    ...
    test('should verify the core requirement: unique levels per rule in ascending order', () {
      ...
      _collectRuleLevels(renderThemeZoomlevel.rulesList, ruleLevels, usedLevels, '');
      ...
      // Verify we have exactly 260 levels (0-259) as stated
      expect(sortedLevels.length, equals(260));
      expect(sortedLevels.first, equals(0));
      expect(sortedLevels.last, equals(259));
