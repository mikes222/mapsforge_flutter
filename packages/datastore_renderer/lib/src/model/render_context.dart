...
class RenderContext {
  ...
  final RenderInfoCollection labels = RenderInfoCollection.empty();
  ...
  void setDrawingLayers(int layer) {
    ...
    currentDrawingLayer = drawingLayers.elementAt(layer);
  }
  ...
  Future<void> initDrawingLayers(SymbolCache symbolCache) async {
    ...
    for (RenderInfo renderInfo in labels.renderInfos) {
      futures.add(renderInfo.createShapePaint(symbolCache));
      if (futures.length > 100) {
        await Future.wait(futures);
        futures.clear();
  ...
  void reduce() {
    for (var layerPaintContainer in drawingLayers) {
  ...
  void statistics() {
    ...
    for (var renderInfo in labels.renderInfos) {
