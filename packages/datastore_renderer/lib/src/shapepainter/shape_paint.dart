...
abstract class ShapePaint<T extends Renderinstruction> {
  final T renderInstruction;

  const ShapePaint(this.renderInstruction);
  ...
  MapRectangle calculateBoundary() {
    return renderInstruction.calculateBoundary();
  }

  void renderNode(UiCanvas canvas, Mappoint coordinatesAbsolute, Mappoint reference, [double rotationRadian = 0]);

  void renderWay(UiCanvas canvas, WayProperties wayProperties, PixelProjection projection, Mappoint reference, [double rotationRadian = 0]);
