...
import 'package:dart_mapfile/src/model/map_header_optional_fields.dart';
...
class MapfileHeaderWriter {
  ...
  void _writeMagicByte(Writebuffer writebuffer) {
    writebuffer.appendStringWithoutLength(MapfileInfoBuilder.BINARY_OSM_MAGIC_BYTE);
  }
  ...
  void _writeMapDate(Writebuffer writebuffer) {
    writebuffer.appendInt8(mapHeaderInfo.mapDate ?? DateTime.now().millisecondsSinceEpoch);
  }

  void _writeBoundingBox(Writebuffer writebuffer) {
    writebuffer.appendInt4(LatLongUtils.degreesToMicrodegrees(mapHeaderInfo.boundingBox.minLatitude));
    writebuffer.appendInt4(LatLongUtils.degreesToMicrodegrees(mapHeaderInfo.boundingBox.minLongitude));
    writebuffer.appendInt4(LatLongUtils.degreesToMicrodegrees(mapHeaderInfo.boundingBox.maxLatitude));
    writebuffer.appendInt4(LatLongUtils.degreesToMicrodegrees(mapHeaderInfo.boundingBox.maxLongitude));
  }
  ...
  void _writeOptionalFlag(Writebuffer writebuffer) {
    int flag = 0;
    if (mapHeaderInfo.debugFile) flag |= MapHeaderOptionalFields.HEADER_BITMASK_DEBUG;
    if (mapHeaderInfo.startPosition != null) flag |= MapHeaderOptionalFields.HEADER_BITMASK_START_POSITION;
    if (mapHeaderInfo.startZoomLevel != null) flag |= MapHeaderOptionalFields.HEADER_BITMASK_START_ZOOM_LEVEL;
    if (mapHeaderInfo.languagesPreference != null) flag |= MapHeaderOptionalFields.HEADER_BITMASK_LANGUAGES_PREFERENCE;
    if (mapHeaderInfo.comment != null) flag |= MapHeaderOptionalFields.HEADER_BITMASK_COMMENT;
    if (mapHeaderInfo.createdBy != null) flag |= MapHeaderOptionalFields.HEADER_BITMASK_CREATED_BY;
    writebuffer.appendInt1(flag);
  }

  void _writeOptionalStartposition(Writebuffer writebuffer) {
    if (mapHeaderInfo.startPosition != null) {
      writebuffer.appendInt4(LatLongUtils.degreesToMicrodegrees(mapHeaderInfo.startPosition!.latitude));
      writebuffer.appendInt4(LatLongUtils.degreesToMicrodegrees(mapHeaderInfo.startPosition!.longitude));
    }
  }
