...
void main() {
  ...
  testWidgets("Split label between two tiles", (WidgetTester tester) async {
    ...
    List<dynamic>? imgs = await (tester.runAsync(() async {
      ...
      datastore.addPoi(PointOfInterest(0, TagCollection(tags: [const Tag('highway', 'turning_circle')]), const LatLong(45.99998, 18.00005)));
      datastore.addPoi(PointOfInterest(0, TagCollection(tags: [const Tag('highway', 'turning_circle')]), const LatLong(46.00006, 18.00005)));
      datastore.addPoi(PointOfInterest(0, TagCollection(tags: [const Tag('highway', 'turning_circle')]), const LatLong(46.00002, 18.00009)));
      datastore.addPoi(
        PointOfInterest(
          0,
          TagCollection(tags: [const Tag('natural', 'peak'), const Tag('name', 'atLeftTile'), const Tag('ele', '5645')]),
          const LatLong(46.00002, 18.00005),
        ),
      );

      //datastore.addPoi(PointOfInterest(0, [Tag('highway', 'turning_circle')], LatLong(46.00000, 18.00007)));
      datastore.addPoi(
        PointOfInterest(0, TagCollection(tags: [const Tag('place', 'suburb'), const Tag('name', 'atRightTile')]), const LatLong(45.99997, 18.00007)),
      );
