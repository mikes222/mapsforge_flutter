...
import 'package:mapsforge_flutter/src/map_position.dart';
...
void main() {
  group('MapPosition', () {
    ...
    test('scaleAround throws for non-positive scale', () {
      expect(() => position.scaleAround(const Offset(0, 0), 0), throwsA(isA<AssertionError>()));

      expect(() => position.scaleAround(const Offset(0, 0), -1.0), throwsA(isA<AssertionError>()));
    });
    ...
    test('rotateTo throws for invalid rotation values', () {
      expect(() => position.rotateTo(-1.0), throwsA(isA<AssertionError>()));

      expect(() => position.rotateTo(360.0), throwsA(isA<AssertionError>()));
    });
