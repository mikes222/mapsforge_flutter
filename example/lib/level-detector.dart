...
class LevelDetector {
  ...
  final BehaviorSubject<SplayTreeMap<int, String>> levelMappings =
      new BehaviorSubject<SplayTreeMap<int, String>>.seeded(
          new SplayTreeMap<int, String>());

  // cache tile level mapping data
  Cache<SimpleTileKey, SplayTreeMap<int, String>> _tileLevelCache =
      new LruCache<SimpleTileKey, SplayTreeMap<int, String>>(
  ...
  LevelDetector(this._viewModel, this._mapDataStore, [this.minZoomLevel = 17]) {
    ...
    this
        ...
        .listen(_getTileCacheData);
    this
        ._viewModel
        .observePosition
        .debounceTime(const Duration(milliseconds: 150))
        .listen(_updateLevelMappings);
  }
  ...
  void _getTileCacheData(MapViewPosition mapViewPosition) {
    if (mapViewPosition.zoomLevel < minZoomLevel) return;

    List<Tile> tiles = LayerUtil.getTiles(
        _viewModel, mapViewPosition, DateTime.now().millisecondsSinceEpoch);
    ...
    if (missingTiles.isNotEmpty)
      compute(readAndProcessTileData,
              new IsolateParam(_mapDataStore, missingTiles))
          .then((tileLevelMappingsBundle) {
  ...
  void _updateLevelMappings([MapViewPosition? mapViewPosition]) {
    ...
    List<Tile> tiles = LayerUtil.getTiles(_viewModel,
        _viewModel.mapViewPosition!, DateTime.now().millisecondsSinceEpoch);

    SplayTreeMap<int, String> combinedLevelMappings =
        new SplayTreeMap<int, String>();
    ...
    if (_viewModel.mapViewPosition!.zoomLevel >= minZoomLevel) {
      for (Tile tile in tiles) {
        SimpleTileKey tileKey = SimpleTileKey(tile);
        SplayTreeMap<int, String>? tileLevelMappings =
            _tileLevelCache.get(tileKey);
...
Future<Map<Tile, Map<int, String>>> readAndProcessTileData(
    IsolateParam isolateParam) async {
  final Map<Tile, Map<int, String>> tileLevelMappingsBundle =
      new Map<Tile, Map<int, String>>();

  for (Tile tile in isolateParam.tiles) {
    final DatastoreReadResult? mapReadResult =
        await isolateParam.mapDataStore.readMapDataSingle(tile);
    ...
    void processTags(List<Tag> tags) {
      ...
      if (levelValue != null) {
        ...
        Iterable<int>? levels =
            IndoorNotationMatcher.parseLevelNumbers(levelValue);
        if (levels != null) {
          // add all levels to the map with empty ref value
          levels.forEach(
              (int level) => tileLevelMappingsBundle[tile]![level] ??= "null");
          // if only one level is given look for further level ref value
          if (levels.length == 1) {
            tileLevelMappingsBundle[tile]![levels.single] ??=
                IndoorNotationMatcher.getLevelRefValue(tags)!;
          }
        }
      }
    }
